 ...
 */
  ...
  return (
    <div className="relative group cursor-pointer overflow-hidden rounded-md h-full min-h-96 w-full hover:shadow-md hover:shadow-green-200 shadow-custom text-gray-200 p-5" onClick={toggleDialog}>

      <div>
        <Image loader={myLoader} src={thumbnail} alt={title} width={360} height={360} className="group-hover:scale-150 transition-all duration-500" />
      </div>
      <div className="absolute left-0 -bottom-[100px] top-0 w-full bg-transparent flex flex-col justify-end items-start group-hover:bottom-0 group-hover:bg-black/30 group-hover:z-20 group-hover:text-white transition-all duration-500 p-3">
        <p className="text-lg font-semibold mb-2 text-emerald-500 group-hover:text-white transition-colors duration-300">{title}</p>
        <div className="">
          <p className="mb-2 line-clamp-2 text-gray-600 group-hover:text-white">{description}</p>
